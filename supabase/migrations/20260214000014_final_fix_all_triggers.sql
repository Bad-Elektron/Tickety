-- FINAL FIX: All trigger functions need SET search_path = public
-- to work correctly when fired by Supabase Auth (GoTrue).
-- Without it, the function can't resolve table names in the trigger context.

-- 1. handle_new_user - creates profile with referral support
...
-- 2. create_default_subscription - creates base tier subscription
...
-- 3. check_pending_offers_on_signup - links pending favor ticket offers
...
DROP TABLE IF EXISTS debug_errors;
DROP FUNCTION IF EXISTS debug_profiles_info();
DROP FUNCTION IF EXISTS debug_auth_triggers();
DROP FUNCTION IF EXISTS debug_function_owner();
