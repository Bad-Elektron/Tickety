...
-- Function to get user's events with server-side filtering and sorting
-- Sorts: upcoming events first (soonest first), then past events (most recent first)
...
-- Create index for efficient queries on organizer's events by date
CREATE INDEX IF NOT EXISTS idx_events_organizer_date
ON events (organizer_id, date DESC)
WHERE deleted_at IS NULL;

-- Create index for text search on title
