...
### 5.5 Referral Deep Linking

**Current state:** Referral codes are shared as plain text. Recipients must manually type the code into the signup form. This works but has friction.

**Phase 1 — Deep links (app installed)**
- [ ] Add `app_links` (Android) / Universal Links (iOS) for `https://tickety.app/invite/{CODE}`
- [ ] Create route handler that extracts the referral code from the URL
- [ ] Navigate to signup screen with referral code pre-filled in the text field
- [ ] Update the share message to include the deep link URL instead of just the code text

**Phase 2 — Landing page (app not installed)**
- [ ] Build a simple web landing page at `https://tickety.app/invite/{CODE}`
- [ ] Page shows: app description, referral benefit, App Store / Play Store buttons
- [ ] "Copy code" button so user can paste it after installing
- [ ] Store the referral code in a server-side table keyed by a short-lived token in the store URL (e.g. `?ref=ABC123XY` as a query param on the store listing)

**Phase 3 — Deferred deep linking (optional, post-launch)**
- [ ] Evaluate Branch.io / AppsFlyer for automatic deferred deep linking
- [ ] If implemented: referral code survives app install and auto-applies on first open
- [ ] Fallback: manual code entry field remains on signup (always keep this)

**Note:** Manual code entry is the most reliable method and should never be removed. Uber, Cash App, and most referral systems kept manual entry for years. Deep linking is a UX improvement, not a replacement.

---

## Phase 6: Platform Admin Dashboard

**Priority: HIGH (Required for operations)**
**Status: NOT STARTED**
**Estimated effort: 3-4 weeks**
**Approach: Next.js + Refine + Supabase Custom Claims**

A dedicated web-based admin panel for platform-wide super-admin access — managing all users, events, payments, and system configuration. Separate from the per-event organizer tools already in the Flutter app.

### Why Next.js (Not Flutter Web)

| Factor | Next.js | Flutter Web |
|--------|---------|-------------|
| **Security** | `service_role` key stays server-side in Route Handlers | Cannot use `service_role` in browser — would need Edge Function proxies for everything |
| **Data visualization** | Recharts, Nivo, Tremor, D3 — mature, rich ecosystem | fl_chart — functional but limited |
| **Admin UI frameworks** | Refine (official Supabase partner), React Admin — auto-scaffold CRUD | Nothing equivalent — every table/form built from scratch |
| **Data tables** | TanStack Table, AG Grid — sorting, filtering, column resize, export | Limited options, significantly more custom work |
| **Deployment** | URL — instant access for any team member | URL (but heavier bundle, slower initial load) |
| **Dev speed** | Days (Refine scaffolds from schema) | Weeks (manual screen-by-screen) |
...
A Supabase Auth Hook function that injects `user_role` into the JWT at login. This lets RLS policies check `auth.jwt() ->> 'user_role' = 'admin'` without extra queries.

#### Admin RLS Policies
An `is_admin()` helper function + admin-level `SELECT`/`UPDATE`/`DELETE` policies on all tables. Admins can read everything; write access scoped per table as needed.
...
create table public.admin_audit_log (
  ...
  action text not null,           -- e.g. 'refund_payment', 'ban_user', 'override_tier'
  target_table text,
  target_id text,
  details jsonb,
...
```
tickety-admin/                    # Separate folder alongside tickety_app/
├── app/
│   ├── api/                      # Next.js Route Handlers (service_role operations)
│   │   ├── users/                # User CRUD, subscription overrides
│   │   ├── events/               # Global event management
│   │   ├── payments/             # Payment queries, refund processing
│   │   └── stripe/               # Stripe dashboard data sync
│   ├── (dashboard)/              # Admin pages behind auth guard
│   │   ├── overview/             # KPIs: revenue, users, tickets sold, active events
│   │   ├── users/                # Search/view any user, edit profile, change tier
│   │   ├── events/               # All events globally, create/edit/delete
│   │   ├── tickets/              # All tickets across events, status, check-in data
│   │   ├── payments/             # All payments, revenue charts, refund tools
│   │   ├── resale/               # Marketplace moderation, seller balances
│   │   ├── subscriptions/        # Tier distribution, upgrades, manual overrides
│   │   ├── referrals/            # Referral chains, earnings, config
│   │   └── settings/             # Platform config (fees, referral %, feature flags)
│   └── layout.tsx                # Auth guard + admin role verification
├── providers/                    # Refine data/auth providers wired to Supabase
├── components/                   # Custom admin widgets, charts, tables
└── package.json
```

### 6.3 Admin Auth Flow

1. Admin logs in with their regular Supabase account (same `auth.users` table as all users)
2. Custom Access Token Hook injects `user_role: 'admin'` into their JWT
3. Client-side Supabase client uses `anon` key — RLS policies enforce admin-level access
4. Operations needing full RLS bypass (e.g., viewing all users, modifying any record) go through Next.js Route Handlers using `service_role` server-side
5. Role change requires re-login (JWT refresh) — acceptable trade-off for simplicity

### 6.4 Admin Screens

| Screen | Purpose | Data Sources |
|--------|---------|--------------|
| **Overview Dashboard** | KPIs, revenue trends, user growth charts, ticket sales graphs | `payments`, `tickets`, `profiles`, `subscriptions` |
| **User Management** | Search/view any user, profile details, subscription tier, referral code, Stripe IDs, ban/unban | `profiles`, `subscriptions`, `auth.users`, `referral_earnings` |
| **Event Management** | All events globally, create/edit/delete, assign organizers, view per-event stats | `events`, `event_staff`, `tickets` |
| **Ticket Overview** | All tickets across all events, filter by status/mode, check-in data, favor ticket offers | `tickets`, `ticket_offers` |
| **Payment Dashboard** | All payments with filters, revenue charts by period, Stripe reconciliation, process refunds | `payments`, `cash_transactions` |
| **Resale Marketplace** | Active listings, moderation tools, seller balances, payout status | `resale_listings`, `seller_balances` |
| **Subscription Management** | Tier distribution pie chart, upgrade/downgrade history, manual tier override | `subscriptions` |
| **Referral Analytics** | Referral chains visualization, total earnings, top referrers, config editor | `referral_config`, `referral_earnings` |
| **Notifications** | Send platform-wide announcements, view delivery stats | `notifications` |
| **System Settings** | Platform fee %, referral percentages, benefit duration, feature toggles | `referral_config`, future `platform_config` table |

### 6.5 Implementation Phases

**Week 1 — Scaffold & Auth**
- [ ] Initialize Next.js project with Refine + Supabase provider
- [ ] Create `user_roles` table + Custom Access Token Hook migration
- [ ] Add admin RLS policies to all existing tables
- [ ] Set up admin auth guard (redirect non-admins)
- [ ] Auto-scaffold CRUD for all 15 tables via Refine
- [ ] Deploy to Vercel (or similar)

**Week 2 — Core Admin Screens**
- [ ] Overview dashboard with KPI cards and revenue/user charts
- [ ] User management — search, view details, edit tier, view payment history
- [ ] Event management — global list, create/edit, view per-event analytics
- [ ] Payment dashboard — transaction table with filters, revenue charts by period

**Week 3 — Advanced Features**
- [ ] Refund processing workflow (trigger existing `process-refund` Edge Function)
- [ ] Resale marketplace moderation (delist, view seller balances)
- [ ] Subscription management (tier distribution, manual override)
- [ ] Referral analytics (chain visualization, config editor)
- [ ] Audit log table + logging on all admin write operations

**Week 4 — Polish & Security**
- [ ] Notifications management (send platform announcements)
- [ ] System settings editor
- [ ] Add `admin_audit_log` writes to all destructive operations
- [ ] Security audit — verify no `service_role` leaks to client
- [ ] Responsive layout for tablet/desktop
- [ ] Testing and bug fixes

### 6.6 Key Technical Decisions

- **Refine over React Admin**: Official Supabase partner, AI scaffolding from schema, headless (use any UI lib), better TypeScript support
- **UI library**: Shadcn/ui + Tailwind CSS (modern, lightweight, customizable) OR Ant Design (richer out-of-box components for admin UIs)
- **Charts**: Recharts (simple, declarative) or Tremor (purpose-built for dashboards)
- **service_role isolation**: All admin mutations go through Next.js Route Handlers — the key never reaches the browser
- **Same Supabase project**: Admin connects to the same database as the Flutter app, not a separate project

### 6.7 Supabase Tables the Admin Needs Access To

| Table | Admin Access Level |
|-------|-------------------|
| `profiles` | Read all, update any (tier override, ban) |
| `events` | Full CRUD globally |
| `event_staff` | Read all, manage any |
| `tickets` | Read all, update status (cancel, refund) |
| `payments` | Read all, trigger refunds |
| `subscriptions` | Read all, override tier |
| `resale_listings` | Read all, delist/moderate |
| `seller_balances` | Read all |
| `cash_transactions` | Read all, reconcile |
| `notifications` | Read all, create platform-wide |
| `notification_preferences` | Read all |
| `ticket_offers` | Read all, cancel |
| `referral_config` | Read/write (singleton) |
| `referral_earnings` | Read all |
| `admin_audit_log` | Read all (own writes automatic) |
| `user_roles` | Read/write (manage admin team) |

---

## Phase 7: Additional Features (Post-Launch)
...
➡️ NEXT:    Phase 5 - Polish & Production
            Launch MVP
⬜ PLAN:    Phase 6 - Platform Admin Dashboard (Next.js + Refine)
            Phase 4 - Crypto Integration
Ongoing:    Phase 7 - Additional Features
```
